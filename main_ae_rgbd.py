...
tf.flags.DEFINE_string("logs_dir", "./logs_ae_rgbd", "path to logs directory")
...
def main(args=None):
    ...
    batch, batch_size = 0, FLAGS.batch_size
    x_batch, y_batch = aaai_parser.load_mat_train_datum_batch(batch * batch_size, (batch + 1) * batch_size)
    x_batch = np.array(x_batch, dtype=np.float32)
    y_batch = np.array(y_batch, dtype=np.int32)
    for xx_idx, xx in enumerate(x_batch):
        scipy.misc.imsave('./test/{:d}_0_rgb.png'.format(xx_idx), xx[:, :, :3])
        scipy.misc.imsave('./test/{:d}_1_s.png'.format(xx_idx), xx[:, :, 3])
        scipy.misc.imsave('./test/{:d}_2_d.png'.format(xx_idx), xx[:, :, 4])
        scipy.misc.imsave('./test/{:d}_3_gt.png'.format(xx_idx), y_batch[xx_idx])
    return
    '''
    # Hyper-parameters
    ...
    with tf.Session() as sess:
        ...
        if FLAGS.mode == 'train':
            ...
            for idx, mat_valid_path in enumerate(aaai_parser.mat_test_paths):
                ...
                sio.savemat('./dataset/AAAI/PHONE_Dnn_rgbd/{}'.format(name), {'a_dict': mat_contents})
