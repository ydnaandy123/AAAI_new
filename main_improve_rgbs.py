...
tf.flags.DEFINE_string("logs_dir", "./logs_improve_rgbs", "path to logs directory")
...
def improve(x, drop_probability, is_training=False):
    with tf.variable_scope("improve"):
        ...
        pool8 = tf.layers.max_pooling2d(inputs=batch_c8, pool_size=[2, 2], strides=[2, 2])
        ...
        """ deconv7 2x2"""
        deconv7 = tf.layers.conv2d_transpose(inputs=pool8, filters=512, kernel_size=[3, 3],
        ...
        concat7 = tf.concat([deconv7_drop, pool7], 3)
        """ deconv6 4x4"""
        ...
        concat6 = tf.concat([deconv6_drop, pool6], 3)
        """ deconv5 8x8"""
        ...
        concat5 = tf.concat([deconv5_drop, pool5], 3)
        """ deconv4 16x16"""
        ...
        concat4 = tf.concat([batch_d4, pool4], 3)
        """ deconv3 32x32"""
        ...
        concat3 = tf.concat([batch_d3, pool3], 3)
        """ deconv2 64x64"""
        ...
        concat2 = tf.concat([batch_d2, pool2], 3)
        """ deconv1 128x128"""
        ...
        concat1 = tf.concat([batch_d1, pool1], 3)
        """ output 256x256"""
    ...
    with tf.Session() as sess:
        ...
        if FLAGS.mode == 'train':
            ...
            for idx, mat_valid_path in enumerate(aaai_parser.mat_test_paths):
                ...
                sio.savemat('./dataset/AAAI/{}_rgbs_improve/{}'.format(
