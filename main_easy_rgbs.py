...
tf.flags.DEFINE_string("logs_dir", "./logs_ss_rgbs", "path to logs directory")
...
def easy(x, drop_probability, is_training=False):
    with tf.variable_scope("easy"):
...
def main(args=None):
    ...
    Network
    """
    logits = easy(x=data_x, drop_probability=drop_probability, is_training=is_training)
    # Loss
    ...
    Optimizer
    """
    trainable_var = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope='easy')
    ...
    with tf.Session() as sess:
        ...
        if FLAGS.mode == 'train':
            ...
            for idx, mat_valid_path in enumerate(aaai_parser.mat_test_paths):
                ...
                sio.savemat('./dataset/AAAI/{}_ss_easy/{}'.format(
