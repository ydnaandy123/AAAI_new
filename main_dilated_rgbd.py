...
tf.flags.DEFINE_string("logs_dir", "./logs_dilated_rgbd", "path to logs directory")
...
def dilated(x, drop_probability, is_training=False):
    with tf.variable_scope("dilated"):
        ...
        """ output 256x256"""
        deconv0 = tf.layers.conv2d_transpose(inputs=concat1, filters=32, kernel_size=[3, 3],
                                             strides=[1, 1], padding='same', activation=tf.nn.relu, name='deconv0')
        batch_d0 = tf.layers.batch_normalization(inputs=deconv0, training=is_training, name='batch_d0')
        output = tf.layers.conv2d(inputs=batch_d0, filters=FLAGS.num_of_class, kernel_size=[1, 1],
...
def main(args=None):
    ...
    Network
    """
    logits = dilated(x=data_x, drop_probability=drop_probability, is_training=is_training)
    # Loss
    ...
    Optimizer
    """
    trainable_var = tf.get_collection(key=tf.GraphKeys.TRAINABLE_VARIABLES, scope='dilated')
        ...
        if FLAGS.mode == 'train':
            ...
            for epoch in range(0, epochs):
                ...
                for batch in range(0, batches):
                    ...
                    x_batch_pre = np.array(x_batch, dtype=np.float32)
                    x_batch = np.concatenate((x_batch_pre[:, :, :, :3],
                                              np.expand_dims(x_batch_pre[:, :, :, 4], axis=3)), axis=3)
                    ...
                    print('global_step: {:d}, epoch: [{:d}/{:d}], batch: [{:d}/{:d}], data: {:d}-{:d}, loss: {:f}'
            ...
            for idx, mat_valid_path in enumerate(aaai_parser.mat_test_paths):
                ...
                x = mat_contents['sample'][0][0]['RGBSD'][:, :, :, :3]
                x_batch_pre = np.array([x], dtype=np.float32)
                x_batch = np.concatenate((x_batch_pre[:, :, :, :3],
                                          np.expand_dims(x_batch_pre[:, :, :, 4], axis=3)), axis=3)
                ...
                sio.savemat('./dataset/AAAI/PHONE_Dnn_rgbsd/{}'.format(name), {'a_dict': mat_contents})
